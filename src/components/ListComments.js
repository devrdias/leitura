						...
						{comments.map(c => (
							<Comment key={c.id}>
								<Comment.Content>
									...
									<Comment.Actions>
										<Comment.Action onClick={e => this.sendVote(e, c.id, 'upVote')}>
											Like
										</Comment.Action>
										<Comment.Action onClick={e => this.sendVote(e, c.id, 'downVote')}>
											Dislike
										</Comment.Action>
										<Comment.Action onClick={e => this.deleteComment(e, c.id)}>
											Delete
										</Comment.Action>
										<Comment.Action onClick={e => this.setState({ comment: c })}>
											Edit
										</Comment.Action>
									</Comment.Actions>
								</Comment.Content>
...
function mapStateToProps({ comments, posts }, { match }) {
	return {
		comments: Object.keys(comments)
			...
			.filter(c => c.parentId === match.params.postId && !c.deleted),
